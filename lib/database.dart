...
class AppDatabase extends _$AppDatabase {
  ...
  Future<List<Task>> getTaskOnDate(DateTime date) {
    return (select(tasks)..where((tbl) => tbl.dueDate.equals(date))).get();
  }
  ...
  Future<List<ActivityData>> getActivityOnDate(DateTime date) {
    return (select(activity)..where((tbl) => tbl.date.equals(date))).get();
  }

  Stream<List<ActivityData>> watchAllActivities() => select(activity).watch();

  Future insertActivity(ActivityData activity_item) =>
      into(activity).insert(activity_item);

  Future updateActivity(ActivityData activity_item) =>
      update(activity).replace(activity_item);

  Future deleteActivity(ActivityData activity_item) =>
      delete(activity).delete(activity_item);

  Future<List<Test>> getAllTests() => select(tests).get();

  Future<List<Test>> getTestOnDate(DateTime date) {
    return (select(tests)..where((tbl) => tbl.date.equals(date))).get();
  }
  ...
  Stream<List<PeriodData>> watchTodayPeriod(int dayNo) {
    return (select(period)..where((tbl) => tbl.day.equals(dayNo))).watch();
  }
  ...
  Future updatePeriod(PeriodData periodData) =>
      update(period).replace(periodData);

  Future deletePeriod(PeriodData periodData) =>
      delete(period).delete(periodData);
}